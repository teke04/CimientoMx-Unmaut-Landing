...
class Controlador_Landings extends Controlador_Admin_Base {
    ...
    private function validarKeyword($keyword, $excludeId = null) {
        // Revisar que la keyword no exista en la base de datos
        if ($excludeId) {
            // Para edición: excluir el registro actual
            $sql_check = "SELECT COUNT(*) FROM landings WHERE keyword = :keyword AND id != :id";
            $params_check = [':keyword' => $keyword, ':id' => $excludeId];
        } else {
            // Para creación: revisar todas las keywords
            $sql_check = "SELECT COUNT(*) FROM landings WHERE keyword = :keyword";
            $params_check = [':keyword' => $keyword];
        }
        
        $result = db()->ejecutarConsulta($sql_check, $params_check);
        if ($result && $result[0]['COUNT(*)'] > 0) {
            return [
                'mensaje' => $excludeId 
                    ? "Otra keyword utiliza el nombre '$keyword'. Por favor, elige otra."
                    : "La keyword '$keyword' ya está registrada. Por favor, elige otra."
            ];
        }
